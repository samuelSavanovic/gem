...
- **`walk_node` / `walk_for_fields` TCO note.** Mirrors `compiler/main.gem`'s `rename_node` — bounded recursion over an AST, so arena pressure is bounded inside one call and the doc-loop back-edge resets between edits. As of 2026-05-09 the diagnostic is a `note:` (not a `warning:`) per option E in `docs/OPTIMIZATIONS.md` "Tighten 'TCO function not reachable from process root' warning"; the structural-decrease check (option B, still TODO) would silence it entirely on bounded walkers. No action needed for correctness.
