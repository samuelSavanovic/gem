...
3. **Conditional-exit process-tail loops** (P1, mid–large). Folds three related items into one session, framed by DX-under-recursion-is-bad (see thinking notes 2026-04-30): users from Ruby/Lua naturally write `while cond` and `break`; today both fail silently (no rescue, no warning) or loudly (refusal). Plumb the post-loop live set as a second input to the backward dataflow; at a `break`, transfer to that set instead of dropping. Tag `while cond` (any cond, not just literal `true`) as process-tail when reachable from a process root — semantically equivalent to `while true; if not cond then break end; ...`, so the same post-loop-set machinery covers both shapes. Update the refusal warning copy to stop suggesting "tail-recursive helper" — under the recursion-is-retiring direction, that advice is itself a DX bug. Hard part is the post-loop set: needs a backward pass at fn scope (not just loop scope) to find live-in of the statements following the loop. Recommend liveness owns the analysis (`compute_live_at_backedge_in_fn(fn_node)` returns a per-loop-node map); codegen looks up by node identity instead of computing post-loop sets itself.
