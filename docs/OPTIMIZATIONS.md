...
3. **`break` in process-tail `while true` body** (P2, mid). Today refused outright — post-loop liveness is not implemented. Unlocks any conditional-exit loop. Plumb the post-loop live set as a second input to the backward dataflow; at a `break`, transfer to that set instead of dropping. The hard part is the post-loop set itself: for a `while true` body inside a fn where break is the only exit, it's the live-in of the statements following the loop in the enclosing fn — needs a backward pass at fn scope, not just loop scope.

4. **Tighten pcall-wrapped spawn closures** (P2, small). `examples/http_server/server.gem:198` is the residual case from the 2026-04-30 spawn tightening: `pcall(fn() spawn(fn() ... end) end)`. The outer pcall closure captures synchronously and the inner spawn deep-copies — neither needs the captures past the call site. Extension is mechanical: recognise `pcall` as a non-escape callee in the same liveness path that already handles spawn, recurse into the closure body for further spawn-with-closure detection. Narrow win; bridges the gap until (5) lands.

5. **Mutation-aware boxing** (P2, large). The principled fix that subsumes (4) and large parts of (1)'s closure-capture refusal. Today every closure-captured var goes into `boxed_vars` (heap-boxed for shared mutability), and any live boxed var refuses the rescue because the box itself lives in the per-process arena. But a var that is only *read* through closures doesn't need a box — closures can deep-copy it at creation time, like spawn already does. Add a pre-pass that classifies each capture site as read-only vs. mutated-through-closure; only the latter is boxed. Bigger refactor (touches closure codegen, capture descriptors, the `make_box` calls), but it's the long-term right answer — the spawn-arg and pcall-arg special cases are both approximations of this. Defer until (1)–(4) and concrete user pain motivate it.

The roadmap above assumes the existing soundness bar (under-rescue = silent memory corruption). Every change must keep the param-superset gate at 4/0 and the negative-test gate green, plus pin a deterministic stress test against the new pattern (cf. `examples/55_while_true_process_loop.gem`).
