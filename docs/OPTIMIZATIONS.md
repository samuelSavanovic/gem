...
4. **Mutation-aware boxing** (P1, large — *promoted from P2*). The principled fix that removes the read-only-capture refusal class entirely (closures that only read captured vars get deep-copy semantics like spawn already does). Today every closure-captured var goes into `boxed_vars` (heap-boxed for shared mutability), and any live boxed var refuses the rescue because the box itself lives in the per-process arena. Add a pre-pass that classifies each capture site as read-only vs. mutated-through-closure; only the latter is boxed. Bigger refactor (touches closure codegen, capture descriptors, the `make_box` calls). Promoted because it's the highest-frequency real-code refusal — any block with a captured var hits it.
