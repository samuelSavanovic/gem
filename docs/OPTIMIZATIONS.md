...
5. **Box relocation on arena reset** (P2, mid — *new*). After (4), the residual refusal class is *mutate-via-block in a process-tail loop* — `let total = 0; arr.each(fn(x) total = total + x end)`. (4) keeps `total` boxed because closures mutate through it; the box lives in the arena; live across the back-edge → refusal. The fix is mechanical, not analysis-heavy: in `gem_arena_reset_with_roots`, walk the boxed-var roots, copy each box's value into a small scratch buffer outside the arena, reset, re-allocate boxes in the fresh arena, copy values back, fix up `GemVal *` in env structs and locals. ~50–100 LOC in the runtime. Cost per reset scales with number of boxed live roots (small in practice). Lifts the last common Ruby/Lua-flavored DX cliff. Defer until (4) lands; only do this if real-code measurement shows the mutate-via-block pattern is common in idiomatic Gem.
...
7. **Lift the main-arena reset invariant** (P1, runtime — *new, surfaced 2026-04-30 by item 3*). When item 3 dropped the `cond_is_true` gates, the obvious next step was to PT-tag top-level loops too — top-level `while`s in user programs (event loops, accept loops without spawn wrappers) should benefit from the rescue. They currently don't: `tag_process_tail_while_loops` calls `walk_for_tagging_stmts(top_stmts, false)`, deliberately scoping top-level out. Reason: `gem_deep_copy_internal` in `runtime/gem_core.c` has a `gem_in_main_arena` fast-path that shares strings allocated in main's arena across processes (cheaper than copying — main never exits). Resetting main's arena would dangle every cross-process string reference. Until the invariant is lifted, top-level loops are a **silent perf cliff**: long-running work at top level grows memory unbounded with no warning. CLAUDE.md forbids that. Two paths: (a) drop the `gem_in_main_arena` fast-path and measure the deep-copy cost (likely small — most cross-process strings are short-lived), or (b) implement a safe main-arena reset by re-anchoring shared strings into a long-lived shared pool before reset. Once landed, drop the `false` argument in `tag_process_tail_while_loops` and add a regression that exercises a top-level process-tail loop (variant of `examples/55_*` without the `spawn` wrapper). Independent of (4); orthogonal sequencing.
...
1. **Rescue codegen references arm-let names not at fn scope** (P1, small). `examples/http_server/server.gem:201` fails to compile after item 4: the rescue emits `&gem_v_fd` where `fd` is declared inside an `else` arm at line 215, so it's in `local_names` (used by the rescue filter) but not a hoisted fn-scope C local — the `&` is unresolved. Pre-item-4 this was masked: `fd` was always boxed (captured by the inner spawn closure), and the rescue refused with *"live var captured by a closure"* before reaching the bad codegen. Item 4 unboxes `fd` (the inner closure only reads it), the refusal lifts, and the latent codegen bug surfaces. Two reasonable fixes: (a) tighten `filter_live_for_rescue` to also reject names that are in `local_names` but not in the fn's hoisted-locals set (refuse with the existing "no C local at this loop's back-edge" reason); (b) extend the existing arm-let hoist pre-pass (`89602f1`-era machinery) to cover lets that are live across an enclosing PT-loop's back-edge. (a) is faster and surfaces the issue as a refusal warning so the user can restructure; (b) is the principled fix and unblocks more code without DX cost. Default to (a) unless (b) is small.
