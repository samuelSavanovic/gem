...
Strategic note (2026-04-30): we are now three speculative fixes deep on the /bookmarks regression — escape analysis (+2.7%), single-copy splice (within noise), and a proposed router freeze. Each fix was justified by a plausible mechanism but the headline barely moves, and the splice introduced its own tradeoff (stranded slack + side-block chain growth). **Stop stacking guesses. Profile first.** Sample one of the http worker pids under `wrk` load (instruments / sample / dtrace) and identify where cycles actually go before writing the next fix. Two options to decide between after profiling: (a) router freeze + companion fixes if deep_copy / rescue cost is still dominant, or (b) revert the per-frame watermark entirely.

Context for option (b): the original motivation for replacing the depth-2 fence with per-frame watermarks was **DX, not performance** — we want Gem to stay simple, and the fence was a codegen wart that made TCO behavior depth-dependent (you couldn't nest TCO loops arbitrarily without losing the reset). The watermark fixes that, but the cost is the /bookmarks regression and the chain of compensating fixes it has spawned. If profiling shows the watermark is structurally expensive on closure-heavy paths regardless of which knob we turn, the right call may be to keep the simpler runtime semantics (depth-2 fence) and accept that nested TCO loops don't reset — that's a smaller DX wart than maintaining splice + freeze + threshold tuning forever.
