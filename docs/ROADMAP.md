...
## Deep non-tail recursion ceiling (P1, was P2)

Every Gem process runs on a fixed-size minicoro stack (currently 256KB, bumped from 16KB for LSP). Only **direct** self-recursive tail calls are TCO'd into `while(1)` loops by the compiler (`is_self_tail_call` in `compiler/codegen.gem` keys on `fn_name == callee.name`). Everything else consumes stack: mutual recursion (`a` tail-calls `b` tail-calls `a` — common in recursive-descent parsers split across helpers), recursion-in-expression-position, and deep visitor patterns over user data. On overflow minicoro logs `coroutine stack overflow, try increasing the stack size` to stderr and the program segfaults shortly after, rather than throwing a catchable error.

**Bumped to P1 after the STOMP broker hit it:** the per-connection writer's `writer_loop ↔ handle_frame ↔ handle_<command> ↔ writer_loop` cycle is mutual tail recursion; it overflows after exactly **175 frames** per connection, taking the broker process down with it. See `examples/stomp_broker/NOTES.md` "Milestone 6: lived experience". The cliff dominates every benchmark in that writeup. Any STOMP-shaped service (state machine over a connection that loops via helper functions) hits this immediately. The `gen_server` and `supervisor` modules dodge it because they happened to be coded as direct self-recursion; broker-shaped code is the more natural pattern and breaks.

Two options worth considering:

- **Compiler-side mutual-tail-call elimination.** Extend the GFP-based TCO analysis (already shipped for direct self-recursion, see `OPTIMIZATIONS_LOG.md` §"Process-loop arena reset / mark_process_tail") to mutual cycles: build the tail-call graph, find SCCs, and codegen the cycle as a single dispatch loop with a per-call-site label. More work than the direct case but in the same conceptual neighborhood. Surfaces compile-time, no runtime cost.
...
- **Catchable stack-overflow error.** Middle option: a guard page below the coroutine stack with a SIGSEGV handler that calls `error("stack overflow in process N")` instead of segfaulting. Lets `pcall` recover and points at the offending function, but still bounds depth at the reservation.

The compile-time analysis is the right first move: it solves the broker case (and any STOMP-shaped state machine) without runtime engineering, and the diagnostic framework from the existing `mark_process_tail` pass extends directly. The catchable-overflow story is still useful for genuinely unbounded recursion (deep user data) and pairs with growable stacks long-term.
