...
/* Like gem_arena_reset_with_roots, but rolls back to a mark taken at TCO
   function entry instead of all the way to a fresh arena. Pre-mark state
   stays intact, so this is safe at any call depth — caller frames above
   the mark may hold GemVals into the arena and they won't dangle.
...
void gem_arena_reset_to_mark_with_roots(GemArenaMark mark, GemVal **roots, int n_roots) {
    ...
    /* Phase 1: stash old arena state, install a fresh side arena in its slot
       so deep_copy allocates into the side. The post-mark region of `old`
       stays mapped and readable for the duration of the rescue copy. */
    GemArena old = *arena;
    gem_arena_init(arena);
    ...
    if (proc) {
        ...
        proc->mailbox.head = NULL;
        proc->mailbox.tail = NULL;
        /* read_buf is arena-allocated and almost always post-mark; nuke it so
           the next tcp_read allocates fresh. Worst case (pre-mark allocation)
           is a one-buffer leak until process exit. */
        proc->read_buf = NULL;
        proc->read_buf_cap = 0;
    }

    GemCopyMap to_side;
    gem_copy_map_init(&to_side, 0);

    for (int i = 0; i < n_roots; i++) {
        *roots[i] = gem_deep_copy_internal(*roots[i], &to_side);
    }

    GemMailbox side_mailbox = {NULL, NULL};
    for (GemMsgNode *n = old_mailbox.head; n; n = n->next) {
        GemVal v = gem_deep_copy_internal(n->value, &to_side);
        GemMsgNode *node = ALLOC(GemMsgNode);
        node->value = v;
        node->next = NULL;
        if (side_mailbox.tail) {
            side_mailbox.tail->next = node;
        } else {
            side_mailbox.head = node;
        }
        side_mailbox.tail = node;
    }

    gem_copy_map_cleanup(&to_side);

    /* Phase 2: truncate `old` to the mark. Tables created after the mark
       (those reachable from old.table_list before mark.table_list in the
       LIFO list) get their malloc'd str_indexes freed; the table_list head
       moves to mark.table_list. Blocks past mark.block get munmapped, and
       mark.block->used rolls back to mark.used. */
    {
        GemTable *t = old.table_list;
        while (t && t != mark.table_list) {
        ...
        old.table_list = mark.table_list;

        if (mark.block) {
            GemArenaBlock *b = mark.block->next;
            while (b) {
                GemArenaBlock *next = b->next;
                size_t total = sizeof(GemArenaBlock) + b->cap;
                munmap(b, total);
                b = next;
            }
            mark.block->next = NULL;
            mark.block->used = mark.used;
            old.current = mark.block;
        }
        old.bytes_allocated = mark.bytes_allocated;
        /* lo/hi may now be wider than the live range, but never narrower —
           gem_in_main_arena's block walk handles correctness; lo/hi is just
           a fast-reject hint. Skip recompute. */
    }
